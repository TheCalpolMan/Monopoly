...
for i in range(-1, 8):
    if 30 + 40 * i == 70:
        playerlist.append(Player(hsltorgb(60, 0.8, 0.5), "Jimmy"))
    else:
        playerlist.append(Player(hsltorgb(30 + 40 * i, 0.8, 0.5), "Jimmy"))
gameinfo = Gamestate(playerlist, "game1", True, True)
for i in range(0, 9):
    gameinfo.players[i].place = 1
cardviewcurrent = 0
buildsetcurrent = 0
    ...
    for event in eventlist:
        if event.type == pygame.QUIT:
            ...
            if event.unicode == "d":
                ...
                toImage(window).save("savestate.png")
            elif event.type == pygame.KEYDOWN:
                if event.unicode == "j":
                    for i in gameinfo.players:
                        i.place = 40
